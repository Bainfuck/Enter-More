...
def complier(指令流: list):
    ...
    while 指令指针 < 指令流长:
        ...
        if 原子==">":
            ...
            栈.append(sum(寄存带[纸带指针]))
        elif 原子 in "Xx":
            for i in reversed(input()):
                栈.append(ord(i))
        elif 原子== "?!":
            print(寄存带[纸带指针][纸带节点指针])
        elif 原子 == "!?":
            try:
                print(chr(栈.pop()), end="")
            except ValueError:
            ...
            栈.append(~(栈[-1] | sum(寄存带[纸带指针])))
        elif 原子 == "!~?":
            tmp = str(~(栈[-1] | int("".join(str(i) for i in 寄存带[纸带指针]))))
            for i in tmp:
                寄存带[纸带指针][纸带节点指针:纸带节点指针+1]=[int(i)]
                纸带节点指针+=1
        elif 原子 == ")":
            if 寄存带[纸带指针][纸带节点指针] != 0:
                回溯指针 = 指令指针 - 1
                while 回溯指针 >= 0:
